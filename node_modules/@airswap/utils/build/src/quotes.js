...
function createQuote(_a) {
    ...
    return __1.lowerCaseAddresses({
        signer: __assign(__assign({}, types_1.emptyQuoteParty), signer),
        sender: __assign(__assign({}, types_1.emptyQuoteParty), sender),
    });
}
...
function getTotalBySignerAmount(quotes) {
    var total = new utils_1.BigNumber(0);
    for (var _i = 0, quotes_1 = quotes; _i < quotes_1.length; _i++) {
        var order = quotes_1[_i];
        total = utils_1.bigNumberify(order.signer.amount).add(total);
    }
    return total;
}
...
function getTotalBySenderAmount(quotes) {
    ...
    for (var _i = 0, quotes_2 = quotes; _i < quotes_2.length; _i++) {
        ...
        total = utils_1.bigNumberify(order.sender.amount).add(total);
    }
    return total;
}
